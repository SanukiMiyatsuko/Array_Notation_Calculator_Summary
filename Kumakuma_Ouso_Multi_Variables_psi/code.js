...
function compute_fund() {
    ...
    Z = { type: "zero" };
    ONE = { type: "psi", arr: Array(lambda).fill(Z) };
    foo = Array(lambda).fill(Z);
    foo[lambda - 1] = ONE;
    OMEGA = { type: "psi", arr: foo };
...
// "0","1","ω"の形式化
let Z = { type: "zero" };
let ONE = { type: "psi", arr: Array(lambda).fill(Z) };
let foo = Array(lambda).fill(Z);
foo[lambda - 1] = ONE;
let OMEGA = { type: "psi", arr: foo };
// オブジェクトの相等判定
function equal(s, t) {
    if (s.type === "zero") {
        return t.type === "zero";
        ...
        if (equal(dom_i_0, ONE)) {
            if (i_0 < lambda - 1) {
                ...
                if (equal(dom(y), ONE)) {
                    ...
                    return plus(fund(x, fund(y, Z)), psi(xArray));
                }
                else {
                    return Z;
                }
            ...
            if (less_than(dom_i_0, x)) {
                ...
                if (equal(dom(y), ONE)) {
                    ...
                    let xArray = [...x.arr];
                    xArray[i_0] = fund(x.arr[i_0], psi(dom_i_0Array));
                    return psi(xArray);
                }
                else {
                    let xArray = [...x.arr];
