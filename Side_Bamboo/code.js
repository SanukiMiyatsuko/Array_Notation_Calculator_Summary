...
function find_parent(s, t) {
    if (s.type == "zero") {
        ...
        return find_parent(s.arr[s.arr.length - 1], t);
    }
    else {
        const sub = s.sub;
        const arg = s.arg;
...
function term_to_string(t) {
    if (t.type == "zero") {
        ...
        return "竹_{" + term_to_string(t.sub) + "}(" + term_to_string(t.arg) + ")";
    }
    else {
...
function abbrviate(str) {
    ...
    if (ABBR_SMALL_OMEGA)
        str = str.replace(/竹_\{0\}\(1\)/g, "ω");
    if (ABBR_LARGE_OMEGA)
        str = str.replace(/竹_\{1\}\(0\)/g, "Ω");
    if(TO_TEX) str = to_TeX(str);
    return str;
}
function to_TeX(str) {
    str = str.replace(/竹/g, "\\textrm{竹}");
...
function leftmost_principal(str) {
    const subpos = serach_closure(str, 0, "{", "}");
    const argpos = serach_closure(str, subpos + 1, "(", ")");
...
function sanitize_string(str) {
    ...
    str = str.replace(/[wω]/g, "b_{0}(b_{0}(0))");
    str = str.replace(/[WΩ]/g, "b_{b_{0}(0)}(0)");
    return str;
}
